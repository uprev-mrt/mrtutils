...
[F] Toggle show-help mode   [?] Symbol info            [/] Jump to symbol
...
def _menuconfig(stdscr):
    ...
    while True:
        ...
        #     _load_dialog()
            ...
            res =  _try_save(_kconf.write_config, _conf_filename, "configuration")
            if res:
                return res
...
def _init():
    ...
    #side bar with description
    _brief_sep_win = _styled_win("separator")
    _brief_win = _styled_win("list")
...
def _resize_main():
    ...
    _brief_sep_win.resize(menu_win_height+1,1)

    if menu_win_height >= 1:
        _menu_win.resize(menu_win_height, int(screen_width/2)-1)
        _help_win.resize(help_win_height, screen_width)
        _brief_win.resize(menu_win_height, int(screen_width/2))

        _top_sep_win.mvwin(1, 0)
        _menu_win.mvwin(2, 0)
        _brief_sep_win.mvwin(1,int(screen_width/2)-1)
        _brief_win.mvwin(2,int(screen_width/2))
...
def _draw_main():
    ...
    if isinstance(node.item, (Symbol, Choice)) and node.help:
        help_lines = []
        split_lines = node.help.split("\n")
        for l in split_lines:
            wrap_lines = textwrap.wrap(l, _width(_brief_win))
            for s in wrap_lines:
                help_lines.append(s)

        for i in range(min(_height(_brief_win), len(help_lines))):
            _safe_addstr(_brief_win, i, 0, help_lines[i])
    else:
        _safe_addstr(_brief_win, 0, 0, "")

    _brief_win.noutrefresh()
    _brief_sep_win.noutrefresh()
